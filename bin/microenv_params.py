...
class MicroenvTab(object):
    ...
    def __init__(self):
        ...
        menv_var2 = Button(description='tnf (mmol)', disabled=True, layout=name_button_layout)
        ...
        self.tnf_diffusion_coefficient = FloatText(value=1200.0,
        ...
        self.tnf_decay_rate = FloatText(value=.0275,
        ...
        self.tnf_initial_condition = FloatText(value=0.0,style=style, layout=widget_layout)
        param_name8 = Button(description='Dirichlet_boundary_condition', disabled=True, layout=name_button_layout)

        self.tnf_Dirichlet_boundary_condition = FloatText(value=0.0,style=style, layout=widget_layout)
        self.tnf_Dirichlet_boundary_condition_toggle = Checkbox(description='on/off', disabled=False,style=style, layout=widget_layout)
        ...
        row_tnf = [menv_var2,  ] 
        row5 = [param_name5, self.tnf_diffusion_coefficient, menv_units_button5]
        row6 = [param_name6, self.tnf_decay_rate, menv_units_button6]
        row7 = [param_name7, self.tnf_initial_condition, menv_units_button7]
        row8 = [param_name8, self.tnf_Dirichlet_boundary_condition, menv_units_button8, self.tnf_Dirichlet_boundary_condition_toggle]
        ...
        box_tnf = Box(children=row_tnf, layout=box_layout)
    ...
    def fill_gui(self, xml_root):
        ...
        self.tnf_diffusion_coefficient.value = float(vp[1].find('.//diffusion_coefficient').text)
        self.tnf_decay_rate.value = float(vp[1].find('.//decay_rate').text)
        self.tnf_initial_condition.value = float(vp[1].find('.//initial_condition').text)
        self.tnf_Dirichlet_boundary_condition.value = float(vp[1].find('.//Dirichlet_boundary_condition').text)
        if vp[1].find('.//Dirichlet_boundary_condition').attrib['enabled'].lower() == 'true':
          self.tnf_Dirichlet_boundary_condition_toggle.value = True
        else:
          self.tnf_Dirichlet_boundary_condition_toggle.value = False
    ...
    def fill_xml(self, xml_root):
        ...
        vp[1].find('.//diffusion_coefficient').text = str(self.tnf_diffusion_coefficient.value)
        vp[1].find('.//decay_rate').text = str(self.tnf_decay_rate.value)
        vp[1].find('.//initial_condition').text = str(self.tnf_initial_condition.value)
        vp[1].find('.//Dirichlet_boundary_condition').text = str(self.tnf_Dirichlet_boundary_condition.value)
        vp[1].find('.//Dirichlet_boundary_condition').attrib['enabled'] = str(self.tnf_Dirichlet_boundary_condition_toggle.value).lower()
